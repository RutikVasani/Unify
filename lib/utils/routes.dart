class MyRoutes {
  static String splashroutes = "/splash";
  static String loginroutes = "/login";
  static String forgotroutes = "/forgot";
  static String mainpageroutes = "/mainpage";
  static String assignmentroutes = "/assignment";
  static String noticeboardroutes = "/noticeboard";
  static String pollingroutes = "/polling";
  static String resumeroutes = "/resume";
  static String settingroutes = "/setting";
  static String syllabusroutes = "/syllabus";
  static String chatroutes = "/chats";
  static String calenderroutes = "/calender";
  static String askqueryroutes = "/askquery";
  static String querySubmittedroutes = "/querySubmitted";
  static String snapshotroutes = "/snapshot";
}
